...
class MyHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      child: PhysicBox(boxPosition: 0.5),
    ));
  }
}

class PhysicBox extends StatefulWidget {
  final double boxPosition;

  PhysicBox({this.boxPosition = 0.0});

  @override
  BoxState createState() => BoxState();
}

class BoxState extends State<PhysicBox> with TickerProviderStateMixin {
  double boxPosition;
  ...
  void initState() {
    ...
    this.boxPosition = widget.boxPosition;
  ...
  void startDrag(DragStartDetails dragStartDetails) {
    start = (context.findRenderObject() as RenderBox).
            globalToLocal(dragStartDetails.globalPosition);

    this.boxPositionOnStart = this.boxPosition;
  }

  void onDrag(DragUpdateDetails dragUpdateDetails) {
    setState(() {
      ...
      final normalDragVel = (dragVelocity / context.size.height).clamp(-1.0, 1.0);
      boxPosition = (boxPositionOnStart + normalDragVel).clamp(0.0, 1.0);
    });
  }
